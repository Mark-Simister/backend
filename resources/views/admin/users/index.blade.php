        ...
        <table id="users-table" class="table table-hover table-bordered align-middle">
            ...
            <tbody>
                ...
                <tr data-user-id="{{ $user->id }}">
                    ...
                    <td>
                        @if($user->image)
                            <img src="{{ asset($user->image) }}" alt="User Image" width="60" height="60" class="rounded-circle">
                        @else
                            <span>No Image</span>
                        @endif
                    </td>
                    <td>
                        ...
                        <form action="{{ route('admin.users.destroy', $user) }}" method="POST" class="d-inline form-delete-user">
                            @csrf
                            @method('DELETE')
    ...
    $(document).on('click', '.btn-toggle-block', function () {
        ...
        Swal.fire({
            ...
            if(result.isConfirmed){
                ...
                $.post(`{{ url('admin/users') }}/${userId}/toggle-block`, { _token: token })
                .done(function(res){
                    if(res.success){
                        // Update button
                        if(res.is_blocked){
